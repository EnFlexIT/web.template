...
export function rawListToTrees(xs: MenuItem[]): MenuTree[] {
     ...
     */
    const sortedXs = xs
        .toSorted((a, b) => getDepthFromList(xs, a.menuID!) - getDepthFromList(xs, b.menuID!))
    ...
    return foldl<MenuTree[], MenuItem>(
        (acc, curr) => (!curr.parentID) ?
            [...acc, { val: curr, children: [] }] :
            acc.map((node) => addNodeToTree(node, {
                children: [],
                val: curr
            })),
        [],
 ...
 */
export function isStaticMenuItem<P>(node: MenuItem<P>): node is StaticMenuItem<P> {
...
export function hasId(tree: MenuTree, id: number): boolean {
    return (tree.val.menuID === id) || foldl<boolean, MenuNode>((acc, curr) => acc || hasId(curr, id), false, tree.children)
}
