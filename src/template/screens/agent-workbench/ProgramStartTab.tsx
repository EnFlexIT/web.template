...
function mapBackendStartModeToUi(
  value: string,
): StartMode {
  switch (value) {
    case "APPLICATION":
...
function mapUiStartModeToBackend(
  value: StartMode,
): string {
  switch (value) {
    case "application":
...
function mapBackendLocalModeToUi(
  value: string,
): LocalAddressMode {
  switch (value) {
...
function mapUiLocalModeToBackend(
...
export function ProgramStartTab() {
  ...
  const startMode =
    mapBackendStartModeToUi(
      settings.startAs,
    );

  const localAddressMode =
    mapBackendLocalModeToUi(
      settings.localMtpCreation,
    );

  const serverMasterUrlParts =
    splitServerMasterUrl(
      settings.serverMasterUrl,
    );

  const urlProtocol =
    settings.serverMasterProtocol ||
    serverMasterUrlParts.protocol;

  const serverIp =
    extractIpAddress(
      serverMasterUrlParts.host,
    );

  const port =
    String(
      settings.serverMasterPort ??
        "",
    );

  const mtpPort =
    String(
      settings.serverMasterPortMtp ??
        "",
    );

  const localIpRaw =
    settings.localMtpUrl ??
    "";
  ...
  const localMtpPort =
    String(
      settings.localMtpPort ??
        "",
    );

  const mtpProtocol =
    settings.localMtpProtocol ??
    "HTTP";
  ...
  const serviceSetupOptions =
    useMemo(
      () =>
        toOptions(
          projectSetups,
          settings.serviceSetup,
        ),
      [
        projectSetups,
        settings.serviceSetup,
  ...
  const backgroundFactoryStateMeta =
    getFactoryStateMeta(
      backgroundFactoryState,
    );
  ...
  const onChangeLocalAddressMode =
    (
      value:
        LocalAddressMode,
    ) => {
  ...
  return (
    ...
    >
      ...
      >
        <View
          style={
            styles.startModeBlock
          }
        >
          <ThemedText
            style={
              styles.startModeTitle
            }
          >
      ...
      <Section
        title="Agent.Workbench Main Server (server.master)"
      >
        <FieldRow
          label="URL / IP"
        >
      ...
      <Section
        title="Own local MTP-Address"
      >
        <View
          style={
            styles.localModeArea
          }
        >
          <View
            style={
              styles.leftLocalMode
            }
          >
            <View
              style={
                styles.radioInlineGroup
              }
            >
              <RadioRow
      ...
      {isBackgroundMode && (
        <View
          style={
            styles.databaseStatusBox
          }
        >
          <View
            style={
              styles.databaseStatusBody
            }
          >
            <AntDesign
              name={
                backgroundFactoryStateMeta
                  .iconName
              }
              size={
                16
      ...
      {isServiceMode && (
        <Section
          title="Agent.Workbench - Service / Embedded System Agent"
        >
          <FieldRow
            label="Project"
          >
          ...
          >
            ...
            >
              ...
              0 ? (
                ...
                settings.embeddedSystemAgents.map(
                  (
                    agent,
                    index,
                  ) => (
