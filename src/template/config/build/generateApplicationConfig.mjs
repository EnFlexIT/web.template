...
/**
 * Discover Application-owned assets automatically.
 */
...
const applicationAssetKeys =
  new Set(
    applicationAssets.map(
      (asset) =>
        asset.propertyName,
    ),
  );
...
/**
 * Validate the optional Application logo against the
 * automatically discovered Application assets.
 */
if (
  applicationLogo &&
  !applicationAssetKeys.has(
    applicationLogo,
  )
) {
  throw new Error(
...
const renderedApplicationAssetsImport =
  applicationLogo
    ? `
import {
  ApplicationImages,
...
const renderedApplicationBranding =
  applicationLogo
    ? `
  branding: {
    logo:
      ApplicationImages.${applicationLogo},
  },
`
    : "";
...
/**
 * Generate Application-owned registries before
